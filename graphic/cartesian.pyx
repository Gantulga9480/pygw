...
cdef class CartesianPlane:
    ...
    def __init__(self, window, (double, double) window_size, Vector2d parent_vector=None, double frame_rate=60, double unit_length=1):
        ...
        if hasattr(window, 'surface'):
            self.window = window.surface
        else:
            self.window = window
        ...
        if parent_vector is not None:
            self.origin = parent_vector._head
        else:
            self.origin = point2d(floor(self.window_size[0] / 2), floor(self.window_size[1] / 2))
        self.set_limit()
    ...
    def X(self):
        ...
        return self.origin._x.num

    @property
    def Y(self):
        if self.parent_vector:
            self.parent_vector.update()
        return self.origin._y.num

    @property
    def ORIGIN(self):
        if self.parent_vector:
            self.parent_vector.update()
        return (self.origin._x.num, self.origin._y.num)
    ...
    def show(self):
        line(self.window, (255, 0, 0), self.origin.get_xy(),
             (self.origin._x.num, self.origin._y.num-10), 2)
        line(self.window, (0, 255, 0), self.origin.get_xy(),
             (self.origin._x.num+10, self.origin._y.num), 2)
    ...
    cpdef double to_X(self, double x):
        if self.parent_vector:
            self.parent_vector.update()
        return self.origin._x.num + x * self.unit_length

    cpdef double to_Y(self, double y):
        if self.parent_vector:
            self.parent_vector.update()
        return self.origin._y.num - y * self.unit_length

    cpdef (double, double) to_XY(self, (double, double) xy):
        if self.parent_vector:
            self.parent_vector.update()
        return (self.origin._x.num + xy[0] * self.unit_length, self.origin._y.num - xy[1] * self.unit_length)

    @cython.cdivision(True)
    cpdef double to_x(self, double X):
        if self.parent_vector:
            self.parent_vector.update()
        return (X - self.origin._x.num) / self.unit_length
    ...
    cpdef (double, double) to_xy(self, (double, double) XY):
        if self.parent_vector:
            self.parent_vector.update()
        return ((XY[0] - self.origin._x.num) / self.unit_length, (self.origin._y.num - XY[1]) / self.unit_length)
    ...
    cdef void set_limit(self):
        if self.parent_vector:
            self.parent_vector.update()
        self.x_min = floor((self.origin._x.num - self.window_size[0]) / self.unit_length)
        self.x_max = floor((self.window_size[0] - self.origin._x.num) / self.unit_length)
        self.y_min = floor((self.origin._y.num - self.window_size[1]) / self.unit_length)
        self.y_max = floor((self.window_size[1] - self.origin._y.num) / self.unit_length)
...
cdef class Vector2d(vector2d):
    ...
    def __init__(self, CartesianPlane plane, double x=1, double y=0, double max=0, double min=0):
        ...
        self.plane = plane
        self._head = point2d(0, 0)
        self.update()

    def __repr__(self):
        self.update()
        return f'X:{self._head._x.num}, Y:{self._head._y.num} - x:{self.get_x()}, y:{self.get_y()}'
    ...
    def TAIL(self):
        self.update()
        return (self.plane.origin._x.num, self.plane.origin._y.num)

    @cython.nonecheck(False)
    @cython.optimize.unpack_method_calls(False)
    def show(self, color=(0, 0, 0)):
        aaline(self.plane.window, color, self.plane.origin.get_xy(), self.get_HEAD())
    ...
    cpdef void random(self):
        ...
        if self.max:
            self.head = ((r1 * 2 - 1) * self.max, (r2 * 2 - 1) * self.max)
        else:
            self.head = ((r1 * 2 - 1) * self.plane.x_max, (r2 * 2 - 1) * self.plane.y_max)

    cpdef void update(self):
        self._head.set_xy(self.plane.to_XY(self.get_head()))
